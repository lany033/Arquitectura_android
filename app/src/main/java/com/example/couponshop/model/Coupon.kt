...
class Coupon(couponJson: JsonObject?) : Serializable {
    ...
    init {
        try {
            ...
            descriptionShort    = descriptionShort.take(5)//chunkWords(couponJson!!.get(DESCRIPTION_SHORT).asString, ' ', 5)
            category            = category.take(1)// chunkWords(couponJson!!.get(CATEGORY).asString, ',', 1)
            ...
            end_date             = getFormatDate(couponJson!!.get(END_DATE).asString)
